...
<div class="row justify-content-center">
    <div class="col-md-12">
        <div class="card">
            <div class="card-body">
                <div class="col-7">
                    <h4>Editar CEPs Bloqueados</h4>          
                </div>
                <form action="{{route('admin.cep_bloqueados.update', $cep_bloqueados->id)}}" id="formStore" method="POST" enctype="multipart/form-data">
                @csrf
                    <div class="row mt-4">
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <select class="form-select" name="id_franqueado">
                                ...
                                @foreach($selecionarFranquia as $franquias)
                                    <option value="{{$franquias->id}}" @if($cep_bloqueados->id_franquia === $franquias->id) selected @endif >{{ $franquias->nome_franquia}}</option>
                                @endforeach
                            </select>
                        </div>
                        @endif
                        <div class="col-12 col-sm-5">
                            ...
                            <div class="input-group ">
                                <input type="text" class="form-control cepMask border-radius-bottom-end-0" name="cep" id="buscaCep" required value="{{$cep_bloqueados->cep}}">
                                <button class="btn btn-outline-primary mb-0" type="button" >  <i class="fa fa-search"></i></button>
                            </div>  
                        </div>
                        <div class="form-group col-sm-6">
                            <span class="titulo"> Rua: *</span>
                            <input id="rua" type="text" name="endereco" class="form-control" placeholder="Pesquisar por rua..." value="{{$cep_bloqueados->endereco}}">
                        </div>
                        <div class="form-group col-sm-6">
                            <span class="titulo"> Bairro: *</span>
                            <input id="bairro" type="text" name="bairro" class="form-control" value="{{$cep_bloqueados->bairro}}">
                        </div>
                        <div class="form-group col-sm-4">
                            <span class="titulo"> Cidade: *</span>
                            <input id="cidade" type="text" name="cidade" class="form-control" value="{{$cep_bloqueados->cidade}}">
                        </div>
                        <div class="form-group col-sm-4">
                            <span class="titulo"> Estado: *</span>
                            <input id="estado" type="text" name="estado" class="form-control" value="{{$cep_bloqueados->estado}}">
                        </div>
                        <div class="form-group col-sm-4">
                            <span class="titulo"> País: *</span>
                            <input id="pais" type="text" name="pais" class="form-control" value="{{$cep_bloqueados->pais}}">
                        </div>
                     </div>
