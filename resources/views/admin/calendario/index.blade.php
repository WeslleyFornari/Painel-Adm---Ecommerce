...
<body class="g-sidenav-show  bg-gray-100">
  ...
  <div class="row mb-3">
	...
	<div class="col-5 d-flex" style="height: 35px;" id="select_produto">
		@include('admin.calendario._selectProdutos')
	</div>
...
(function () {
	...
	scheduler.templates.event_bar_text = function (start, end, event) {
		...
		var pedido = "name='pedido_" + event.id + "' value='" + estoque.label + "'";
	...
	scheduler.templates.tooltip_text = function (start, end, event) {
		...
		html.push("Data Entrega: <b>" + eventDateFormat(start) + "</b>");
...
$("body").on('change', '#franquiaid', function (e) {
	...
	var url = '{{ route("admin.calendarios.selectProduto", ["id" => ":id"]) }}';
	url = url.replace(':id', id);

	$.get(url, function(data) {
		$("#select_produto").html(data);
	});
});
...
$("body").on('change', '#produtos', function (e) {
	...
	var url = '{{ route("admin.calendarios.estoques", ["id" => ":id", "id_produto" => ":id_produto"]) }}';
    url = url.replace(':id', id_pedido).replace(':id_produto', id_produto);
