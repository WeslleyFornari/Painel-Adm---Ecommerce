                     ...
                     <div class="col-3">{{$item->email ?? ''}}</div>
                      ...
                      <div class="col-1 justify-content-center d-flex">
                         ...
                         <div class="col-2 text-center">
                            @if(permissao('usuarios')->visualizar == 'sim')
                               <a href="{{ route('admin.usuarios.preview',$item->id) }}"class="btn btn-info btn-sm show btn-icon-only preview-usuario"><i class="fas fa-eye"></i></a>
                            @endif
                            @if(permissao('usuarios')->editar == 'sim')
                                <a href="{{route('admin.usuarios.edit', $item->id)}}" class="btn btn-sm btn-icon-only btn-secondary editar-usuarios"> <i class="fas fa-pencil"></i> </a>
                            @endif
                            @if(permissao('usuarios')->deletar == 'sim')
                                <a href="{{route('admin.usuarios.delete', $item->id) }}" class="btn btn-sm btn-icon-only btn-danger btn-destroy"> <i class="fas fa-trash"></i> </a>          
                            @endif
                        </div>   
                </div>
                @endforeach  
