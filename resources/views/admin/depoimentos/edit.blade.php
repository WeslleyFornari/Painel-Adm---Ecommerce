...
<form id="formStore" action="{{route('admin.depoimentos.update', ['id' => $depoimento->id])}}" method="POST" enctype="multipart/form-data">
@csrf
                ...
                <div class="row">
                        ...
                        <div class="form-check form-switch">
                                <input class="form-check-input status" type="checkbox" name="status" role="switch" value="ativo"
                                    data-id="{{$depoimento->id}}" @if($depoimento->status == 'ativo') checked @endif>
                        ...
                        <div class="form-group inline col-sm-4 d-flex align-items-center">
                            ...
                            <select name="tipo_franqueado" required id="" class="form-select">
                                ...
                                <option value="toy" @if($depoimento->tipo_franqueado == 'toy') selected @endif>TOY</option>
                                <option value="trip" @if($depoimento->tipo_franqueado == 'trip') selected @endif>TRIP</option>
                            </select>
                        </div>  
                   </div>
                    ...
                    <!-- CONTEUDO SUMMER NOTE -->
                        <div class="col-8">                               
                                <textarea id="summernote" name="texto" class="form-control" style="display: block;">{{$depoimento->texto}}</textarea>
                        </div>
                        ...
                        <div class="col-4 text-center">
                            ...
                            <x-upload-file target="logo" collum="id_foto" type="single" :media="$depoimento"/>
                        </div>
